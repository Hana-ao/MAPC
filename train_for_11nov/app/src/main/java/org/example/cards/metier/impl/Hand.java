...
public class Hand implements Iterable<ICard>, Comparable<Hand> {
...
*/
    ...
    public static Comparator<Hand> compareByNumberOfJokers(){ //ne prend pas d'arguments, mais compare, la méthode de l'objet Comparator, en prend
        /*	•	Signature de la méthode : public static Comparator<Hand> getComparatorByHearts()
	•	Elle retourne un Comparator spécifique qui compare des objets de type Hand.
	•	Lambda : (hand1, hand2) -> { ... }
	•	Cette expression lambda remplace l’implémentation de compare(T o1, T o2) et prend deux mains hand1 et hand2 en argument pour les comparer.
        En gros le comparator permet de comparer deux objets (ici des mains) selon un certain ordre et d'en return (indirectement) le résultat
        VOIR EN DESSOUS si tu comprends PAS */

        return (hand1, hand2) -> {
            int nbJokers1 = 0;
            int nbJokers2 = 0;

            for(ICard elt1 : hand1.hand){
                if(elt1 instanceof Joker){nbJokers1++;}
            }
            for(ICard elt2 : hand2.hand){
                if(elt2 instanceof Joker){nbJokers2++;}
            }
            return Integer.compare(nbJokers1, nbJokers2);
        };
    }

    public static Comparator<Hand> compareByDiamonds(){
        return (hand1, hand2) -> {
            int nbDiam1 = (int) hand1.hand.stream().filter(card -> card.getSuit() == Suit.DIAMONDS).count();
            int nbDiam2 = (int) hand2.hand.stream().filter(card -> card.getSuit() == Suit.DIAMONDS).count();
